...
int main(int argc, char ** argv)
{
	...
	if (flags.flagError)
	{
		cout << "try 'ph+ -h' for help" << endl;
		return 0;
	}
	
	if (flags.help)
	{
		cout << "PH+ v" << VERSION_X << "." << VERSION_Y << "." << VERSION_Z << endl;
		cout << "usage: ph+ [options] [source file] [options]" << endl;
		cout << "options: " << endl;
		cout << "-v, -version      display the version of ph++" << endl;
	...
	if (flags.version)
	{
		cout << "ph+ version " << VERSION_X << "." << VERSION_Y << "." << VERSION_Z << endl;
		return 0;
	}
	...
	{
		cout << "multiple source files specified, ph+ does not currently support this" << endl;
		cout << "try 'ph+ -h' for help" << endl;
		return 0;
	}
		...
		else
		{
			...
			if (!flags.binOutFile.empty() || flags.runCompiled)
			{
				...
				if (flags.runCompiled)
				{
					...
					cmd = "./"+binFileName + " --running-from-ph+ " + str::join(cmdLineArgs, " ", false);
